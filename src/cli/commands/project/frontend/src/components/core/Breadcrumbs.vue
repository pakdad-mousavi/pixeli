  ...
  <div class="flex items-center text-xs gap-x-1 w-full breadcrumbs">
    ...
    <div class="w-full">
      ...
      <div v-else class="flex items-center gap-x-1 w-full">
        <div v-for="(crumb, index) in crumbs" class="flex items-center gap-x-1 text-zinc-500 dark:text-mist-500">
          <span
            class="line-clamp-1"
            :class="{
      ...
      <div v-if="crumbs.length === 0">
        <span class="text-mist-800/60 dark:text-mist-400 font-mono">./</span>
      </div>
    </div>
  </div>
</template>
